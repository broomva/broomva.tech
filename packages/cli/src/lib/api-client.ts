...
import type { PromptDetail, PromptSummary, SkillDetail, SkillsResponse, ContextResponse } from "../types/api.js";
...
export class ApiClient {
  ...
  private async request<T>(
    method: string,
    path: string,
    opts: { body?: unknown; auth?: boolean; params?: Record<string, string> } = {},
  ): Promise<T> {
  ...
  async listPrompts(filters?: {
    ...
    if (filters?.mine) {
      return this.request<PromptSummary[]>("GET", "/api/prompts/user", { auth: true });
    }
    return this.request<PromptSummary[]>("GET", "/api/prompts", { params });
  }

  async getPrompt(slug: string): Promise<PromptDetail> {
    return this.request<PromptDetail>("GET", `/api/prompts/${encodeURIComponent(slug)}`, {
      params: { format: "full" },
    });
  }
  ...
  async updatePrompt(
    ...
    return this.request<PromptDetail>("PUT", `/api/prompts/${encodeURIComponent(slug)}`, {
      body: data,
      auth: true,
    });
  }
  ...
  async deletePrompt(slug: string): Promise<void> {
    await this.request<unknown>("DELETE", `/api/prompts/${encodeURIComponent(slug)}`, {
      auth: true,
    });
  }
  ...
  async getSkill(slug: string): Promise<SkillDetail> {
    return this.request<SkillDetail>("GET", `/api/skills/${encodeURIComponent(slug)}`);
  }
