...
Only canonical deliverables are committed. Iteration binaries (Seedream attempts 01–04, hero v01–v04, social v01–v02, Kling attempts 01–03 + pre-overlay base) are intentionally not in the repo to keep the asset pack lean — each canonical asset's iteration count is preserved via JSON provenance files (`seedream-NN.json`, `kling-NN.json`) and the production log in `docs/superpowers/plans/2026-05-11-broomva-commercial-visual-production.md`.

- `substrate/` — `locked.png` (2560×1440, the AI-generated substrate that became the canonical base for hero / social / video); `locked-square.png` (1440×1440 center crop used as the social starting point); `seedream-01.json` through `seedream-04.json` (provenance of the 4 iterations that produced the lock).
- `hero/` — `hero.png` (canonical 16:9 composite with type).
- `social/` — `social.png` (canonical 1:1 composite with type).
- `video/` — `video.mp4` (canonical 1920×1080, 10s, H264+AAC, with type overlay fade-in over the last 2 seconds); `overlay.png` (transparent type bed used by ffmpeg); `video-frame0.png` (first frame, depopulated substrate — used in the contact sheet); `kling-NN.json` (provenance of the 3 Kling 2.6 iterations); `cinema-preflight.json` (records that Cinematic Studio Video 3.0 was Pro-plan-gated, so Kling carried the whole video pass).
- `svg/` — SVG templates that overlay the type onto the substrate (Cardo + GFS Didot + STIX Two via system fonts; substrate base64-embedded to work around `rsvg-convert`'s cross-directory restriction). `hero-plate.svg`, `social-plate.svg`, `video-overlay.svg`.
