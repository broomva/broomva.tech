# Design System: Arcan Glass (superseded)

> **Superseded on 2026-08-10 by the Broomva blue-axis system** (BRO-2106). The
> canonical contract now lives in the repository root `DESIGN.md`, materialized
> from the `broomva-design` skill. This file is retained for provenance: it
> documents the visual identity broomva.tech carried before the migration.
>
> Note that parts of this document were already stale against the implementation
> when it was superseded. `--ag-ai-blue` had drifted to `oklch(0.60 0.12 260)`
> and `--ag-accent-blue` to `oklch(0.65 0.14 235)`, and Phosphor Web3 Green had
> already been demoted from a brand accent to the functional `--ag-success`
> token. Read the git history of `apps/broomva/app/globals.css`, not this file,
> for what was actually rendering.
