...
description: "The Life Agent Operating System — active subsystems modeled after biological primitives, written in Rust."
---
...
The system is organized as a Rust workspace with 12 active subsystems across 139 packages. See the mirrored workspace overview and repository metadata for current measured counts; these figures change as the workspace evolves. Most crates use Rust 2024 Edition with MSRV 1.85, while target-specific exceptions are documented with their packages.
...
Instead of operating the infrastructure yourself, you can deploy a managed Life instance through the [Broomva platform](/platform/deployments). The platform handles provisioning, persistence, monitoring, and scaling.
