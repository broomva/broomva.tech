...
function toSummary(kind: ContentKind, slug: string, frontmatter: ContentFrontmatter): ContentSummary {
...
async function readFile(kind: ContentKind, slug: string): Promise<string | null> {
...
export async function getContentList(kind: ContentKind): Promise<ContentSummary[]> {
...
export async function getContentBySlug(kind: ContentKind, slug: string): Promise<ContentDocument | null> {
	const raw = await readFile(kind, slug);
	if (!raw) return null;
...
export async function getLatest(kind: ContentKind, limit = 3): Promise<ContentSummary[]> {
	const list = await getContentList(kind);
	return list.slice(0, limit);
}

export async function getPinnedProjects(limit = 3): Promise<ContentSummary[]> {
