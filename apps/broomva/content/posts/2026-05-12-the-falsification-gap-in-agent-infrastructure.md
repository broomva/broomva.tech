...
| Opus-4-7 | 0.495 ± 0.079 | −0.000 | +0.141 | +0.131 | 0.158 | helps directionally, n=3 inconclusive |

**Read this table carefully.** The only cells whose confidence intervals don't include zero are the three Sonnet cells, where recursion *measurably hurts*. Haiku is noise. Opus is directional but the per-seed σ of 0.079 swamps the n=3 mean shift. A `--paper` run (n=20) at Opus remains the cleanest path to either confirming or burying the Opus positive.
...
**Most agent-infrastructure claims are currently unfalsifiable as stated.** When a vendor says "our framework improves agent performance," the claim is consistent with both our Sonnet result (it measurably hurts) and our Opus result (it directionally helps but inconclusive at n=3). Without published paired ablations at multiple tiers, the claim is not wrong — it is *non-evaluable*.
...
1. **Default-config n=20 at Opus.** Tightens the Opus directional positive to either a confirmed help or a buried null. The bench is running as I publish this. ~$420 spend, ~24–36h wall, apples-to-apples vs the PR #31 n=3 baseline. A first attempt at a `--paper` mode run was halted at $1.50 when the dispatched agent caught that `--paper` doesn't just increase seeds — it switches the per-seed config from 90 to 1,000 episodes (a 9× cost blow-up to ~$4,500). The corrected default-config run is the right experiment.
