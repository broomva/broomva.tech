...
summary: "Git is distributed and decentralized by design — GitHub just happens to be the centralized layer we chose to put on top."
date: 2020-03-01
published: true
tags:
  - git
  - distributed-systems
  - decentralization
audio: /audio/writing/git-decentralization.mp3
---

![Decentralized peer-to-peer mesh network of equal repository nodes with branching commit histories](/images/writing/git-decentralization/hero-opt.png)

A reminder that keeps needing to be said: Git and GitHub are not the same thing, and the difference matters more than most people realize.
...
Also, git can communicate with any host, it is in no way dependent on GitHub to provide centralization, even though we often treat it as if that were the case. Git can use SSH, HTTP(S), and even its own proprietary protocol to push and fetch data from a repo on any other system, provided the user has the ability to log in to that host. This is not particularly comparable to blockchain.
