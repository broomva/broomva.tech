...
<video src="/video/writing/bstack-portable-harness-metalayer/inflection.mp4" poster="/images/writing/bstack-portable-harness-metalayer/chart-inflection.png" autoplay muted loop playsinline preload="metadata" aria-label="Throughput inflection: yearly contributions 2022-2026 + monthly breakdown of 2026, showing a 6.2x jump in March 2026 when bstack started shipping" style="width:100%;border-radius:12px;margin-bottom:1.5rem;background:#04060d"></video>
...
If you build with Claude Code, Codex, Gemini CLI, OpenCode, or any of the [50+ agent CLIs the skills ecosystem supports](https://github.com/vercel-labs/skills), you can install the whole stack right now:
...
The full eleven-primitive spec — what each closes, how it's enforced, and the four reasoning-enforced reflexive trigger rules — lives at [`references/primitives.md`](https://github.com/broomva/bstack/blob/main/references/primitives.md). The repo itself is [broomva/bstack](https://github.com/broomva/bstack).
...
| **P9** | CI Watcher + Productive Wait (`broomva/p9` skill) | Agents `sleep` on CI and waste 5–15 minutes per PR. | `gh pr checks --watch` via `run_in_background`. Wait-queue drains during the wait. Classifier + evaluator self-heal red CI. Auto-merge actuator with governance pre-pass. |
...
bstack does this with **reflexive trigger rules** in AGENTS.md, written as binding-on-every-agent text. P6 has one ("Bookkeeping is reflexive — invoke without being prompted whenever you commit a feature that reads from the graph"). P9 has four ("After every push, before any sleep, on red CI before re-pushing, on MERGE_READY before manual merge"). The rule loads at SessionStart; the agent reads it and applies it. No hook needed. If the agent finds itself sleeping on CI, the reflex failed and that's a session-level error worth flagging.
...
The install is at the top. The full primitive spec is at [`references/primitives.md`](https://github.com/broomva/bstack/blob/main/references/primitives.md). The repo is [broomva/bstack](https://github.com/broomva/bstack). The RCS formalization with stability proofs lives in `research/rcs/papers/p0-foundations`.
